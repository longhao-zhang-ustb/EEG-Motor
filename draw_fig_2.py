...
if __name__ == '__main__':
    ...
    ###############################计算方差贡献率的相关代码####   ##########################
    single_var_ratio = pca.explained_variance_ratio_
    # 累计方差贡献率
    cum_var_ratio = np.cumsum(single_var_ratio)
    # 获取30和45对应的累计贡献率
    cum_var_ratio_30 = cum_var_ratio[29]
    cum_var_ratio_45 = cum_var_ratio[44]
    # 5. 绘图：柱状图+累计折线图
    fig, ax = plt.subplots(figsize=(18, 5))
    # 单个贡献率柱状图
    ax.bar(
        x=np.arange(1, len(single_var_ratio)+1),
        height=single_var_ratio,
        color="black",
        edgecolor='black',
        linewidth=0.4,
        alpha=0.6,
        label='Individual variance contribution'
    )
    # 累计贡献率折线
    plt.plot(
        np.arange(1, len(cum_var_ratio)+1),
        cum_var_ratio,
        color='#D84452',
        linewidth=2,
        marker='o',
        markersize=3,
        label='Cumulative variance contribution'
    )
    # 标记下30和45对应的累计贡献率
    plt.vlines(x=30, ymin=0, ymax=cum_var_ratio_30, linestyle='--', color='black', alpha=0.4, label='First 30 principal components')
    plt.vlines(x=45, ymin=0, ymax=cum_var_ratio_45, linestyle='-.', color='purple', alpha=0.4, label='First 45 principal components')
    # 标记具体数值
    plt.text(29, cum_var_ratio_30 + 0.01, f'{cum_var_ratio_30:.4f}', fontsize=16, fontweight="bold")
    plt.text(44, cum_var_ratio_45 + 0.01, f'{cum_var_ratio_45:.4f}', fontsize=16, fontweight="bold")
    # 设置刻度字体
    plt.tick_params(axis='both', labelsize=18)
    # 设置刻度从1开始
    ax.set_xticks(np.arange(1, len(cum_var_ratio)+1, 1))
    plt.xlabel('Principal components', fontsize=18, fontweight='bold')
    plt.ylabel('Variance contribution ratio', fontsize=18, fontweight='bold')
    # 挪动一下位置
    plt.legend(bbox_to_anchor=(0.6, 0.5),fontsize=18)
    ...
    plt.show()
    # 保存图片
    # plt.savefig(f'paper_figures\\variance_contribution_{band_name}_{feature_name}.tif', dpi=300, bbox_inches='tight')
    exit()
    ######################################################################################
    """
