...
if __name__ == '__main__':
    ...
    for index, test_pid in enumerate(unique_pids):
        ...
        # 保存模型
        joblib.dump(model, f'experiment7_models\\svm_patient_{index}_{seed}_{band_name}_{feature_name}.pkl')
        print(f'预测完患者: {test_pid}，数据条数：{len(X_test)}')
    exit()
    ##################################################################################################################
    # # 计算svm的系数和权重, 计算所有正确的模型的系数和权重
    ...
    X_train = pd.DataFrame(
        X_train, 
        columns=[f'PCA component {i}' for i in range(X_train.shape[1])]
    )
    # 解释器
