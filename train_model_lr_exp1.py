...
if __name__ == '__main__':
    ...
    # 绘制ROC曲线的参数
    fpr_lr, tpr_lr, thresholds_roc_lr = roc_curve(y_true, y_pred_proba)
    roc_auc = auc(fpr_lr, tpr_lr)
    # 绘制PR曲线的参数
    precision_lr, recall_lr, thresholds_pr_lr = precision_recall_curve(y_true, y_pred_proba)
    ap_lr = average_precision_score(y_true, y_pred_proba)
    # 结果保存到txt文件
    with open(f'experiment1_res\\lr_{band_name}_results.txt', 'w') as f:
        # 预测标签，预测概率，真实标签写入文件
        ...
        # 保存ROC和PR参数
        f.write(f"ROC fpr: {fpr_lr.tolist()}")
        f.write("\n")
        f.write(f"ROC tpr: {tpr_lr.tolist()}")
        f.write("\n")
        f.write(f"ROC thresholds: {thresholds_roc_lr.tolist()}")
        f.write("\n")
        f.write(f"ROC AUC: {roc_auc}")
        f.write("\n")
        f.write(f"PR precision: {precision_lr.tolist()}")
        f.write("\n")
        f.write(f"PR recall: {recall_lr.tolist()}")
        f.write("\n")
        f.write(f"PR thresholds: {thresholds_pr_lr.tolist()}")
        f.write("\n")
        f.write(f"PR AP: {ap_lr}")
    # 绘制混淆矩阵
    ...
    # plt.show()
    # 保存为dpi300的tif
    plt.savefig(f'experiment1_res\\lr_{band_name}_confusion_matrix.tif', dpi=300)
    exit()
    # 绘制ROC曲线
    ...
    plt.tight_layout()
    # 保存为dpi300的tif
    plt.savefig(f'experiment1_res\\lr_{band_name}_ROC_curve.tif', dpi=300)
    # 绘制PR曲线
    plt.figure(figsize=(10, 6))
    ...
    plt.tight_layout()
    # 保存为dpi300的tif
    plt.savefig(f'experiment1_res\\lr_{band_name}_PR_curve.tif', dpi=300)
