...
if __name__ == '__main__':
    ...
    try:
        # 删除之前的结果文件
        os.remove(f'experiment3_res\\svm_{band_name}_{feature_name}__results.txt')
    except:
        pass
    # 遍历不同维度，记录每个维度的结果
    # 核心：逐个患者留一！！！
    for dim in range(1, 78):
        # 对于同一个患者，遍历不同维度，记录每个维度的结果
        y_pred = []
        y_true = []
        y_pred_proba = []
        ...
        with open(f'experiment3_res\\svm_{band_name}_{feature_name}__results.txt', 'a+') as f:
