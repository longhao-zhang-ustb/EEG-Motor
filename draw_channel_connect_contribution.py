...
if __name__ == '__main__':
    ...
    # ===================== 计算每个通道在 coherence 中的总贡献 =====================
    # 思路：每个 coh 特征是通道对，贡献平分给两个通道
    chan_contrib = np.zeros((len(channels), pca.n_components))

    for i, feat in enumerate(original_feature_names):
        if 'coh' in feat and band_name in feat:
            # 找出这个相干特征包含哪些通道
            involved = [ch for ch in channels if ch in feat]
            if len(involved) == 2:
                ch1, ch2 = involved
                w = np.abs(loadings[i])  # 该相干特征对各PC的载荷绝对值

                idx1 = channels.index(ch1)
                idx2 = channels.index(ch2)

                # 两个通道各分一半贡献
                chan_contrib[idx1] += w / 2
                chan_contrib[idx2] += w / 2

    # 按 PC 归一化到 0~1
    total = chan_contrib.sum(axis=0)
    contrib_matrix = chan_contrib / total
    ...
    bottom = np.zeros(pca.n_components)
    ...
    ax.set_xlabel('Principal components', fontsize=14, fontweight='bold', labelpad=15)
    ax.set_ylabel('Proportion of contribution', fontsize=14, fontweight='bold', y=0.49)
    # 坐标刻度字体大小
    ax.tick_params(axis='both', labelsize=14)
    ax.set_xticks(range(1, pca.n_components+1, 2))
    ax.set_ylim(0, 1)
    ...
    plt.tight_layout()
    #保存为300dpi的tif图片
    plt.savefig(f'experiment6_res\\pca_channel_connect_contribution_patient_{index}_{band_name}_{feature_name}.tif', dpi=300, bbox_inches='tight')
