...
if __name__ == '__main__':
    ...
    # feature_cols = [col for col in df.columns if f'{band_name}_psd' in col]
    # feature_name = 'psd'
    # feature_cols = [col for col in df.columns if f'{band_name}_coherence' in col]
    # feature_name = 'coherence'
    # feature_cols = [col for col in df.columns if 'crs_diff_2' in col]
    # feature_name = 'crs_2'
    # feature_cols = [col for col in df.columns if f'{band_name}_psd' in col or f'{band_name}_coherence' in col]
    # feature_name = 'psd_coherence'
    # feature_cols = [col for col in df.columns if f'{band_name}_psd' in col or 'crs_diff_2' in col]
    # feature_name = 'psd_crs_2'
    feature_cols = [col for col in df.columns if f'{band_name}_coherence' in col or 'crs_diff_2' in col]
    feature_name = 'coherence_crs_2'
    X = df[feature_cols]
    y = df['label']
    ...
    # 结果保存到txt文件
    with open(f'experiment2_res\\svm_{band_name}_{feature_name}__results.txt', 'w') as f:
        # 预测标签，预测概率，真实标签写入文件
    ...
    sns.heatmap(
        ...
        annot=True,        # 显示数字
        fmt="d",           # 整数格式
        cmap="Reds",      # 颜色：Blues / Reds / Greys 任选
        linewidths=0.1    # 边框线
    )
    plt.xlabel("Predicted State", fontsize=14, fontweight='bold')
    plt.ylabel("True State", fontsize=14, fontweight='bold')
    plt.tight_layout()
    # 保存为dpi300的tif
    plt.savefig(f'experiment2_res\\svm_{band_name}_{feature_name}__confusion_matrix.tif', dpi=300)
    # 绘制ROC曲线
    ...
    plt.tight_layout()
    # 保存为dpi300的tif
    plt.savefig(f'experiment2_res\\svm_{band_name}_{feature_name}__ROC_curve.tif', dpi=300)
    # 绘制PR曲线
    plt.figure(figsize=(10, 6))
    ...
    plt.tight_layout()
    # 保存为dpi300的tif
    plt.savefig(f'experiment2_res\\svm_{band_name}_{feature_name}__PR_curve.tif', dpi=300)
